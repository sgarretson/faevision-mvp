  ...
  try {
    ...
    const adminUser = await prisma.user.create({
      data: {
        ...
        password:
          '$2a$10$rQZ8vJKnZVzQ4X5N6bGGy.CXjRO9kJGzJy2xhVz3wJ1FH5XQ9YWQK', // demo123 hashed
      },
    });
    ...
    const testInput = await prisma.input.create({
      data: {
        ...
        description:
          'Current API response times affecting executive dashboard performance',
        department: 'ENGINEERING',
    ...
    const testSolution = await prisma.solution.create({
      data: {
        ...
        businessValue:
          'Improved executive experience with faster response times',
        successMetrics: [
          'Response time <500ms',
          'Zero timeouts',
...
async function createTestAuthState(baseURL: string) {
  ...
  try {
    ...
    await executivePage.fill(
      '[data-testid="email"]',
      'sarah.chen@faevision.com'
    );
    ...
    await executiveContext.storageState({
      path: 'tests/auth/executive-auth.json',
    });
  } catch (error) {
  ...
  try {
    ...
    await contributorPage.fill(
      '[data-testid="email"]',
      'alex.thompson@faevision.com'
    );
    ...
    await contributorContext.storageState({
      path: 'tests/auth/contributor-auth.json',
    });
  } catch (error) {
