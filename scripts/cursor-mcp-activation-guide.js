...
async function runActivationGuide() {
  try {
    ...
    const cursorOpen = await askQuestion(
      '1. Is Cursor IDE currently open? (y/n): '
    );
    if (cursorOpen !== 'y') {
    ...
    const extensionsOpen = await askQuestion(
      '2. Can you access Extensions (Cmd/Ctrl + Shift + X)? (y/n): '
    );
    if (extensionsOpen === 'y') {
      console.log(
      ...
      const mcpFound = await askQuestion(
        '3. Do you see MCP or Model Context Protocol extensions? (y/n): '
      );
      if (mcpFound === 'y') {
    ...
    const settingsAccess = await askQuestion(
      '4. Can you access Cursor Settings (Cmd/Ctrl + ,)? (y/n): '
    );
    if (settingsAccess === 'y') {
      console.log(`\n✅ Great! Search for "MCP" in settings.`);

      const mcpSettings = await askQuestion(
        '5. Do you see MCP or Model Context Protocol settings? (y/n): '
      );
      if (mcpSettings === 'y') {
...
Configuration to add:
{
  "mcpServers": {
    ...
    const restartReady = await askQuestion(
      '6. Ready to restart Cursor and test? (y/n): '
    );
    if (restartReady === 'y') {
      ...
      const testingDone = await askQuestion(
        '7. Have you completed the restart and testing? (y/n): '
      );
      if (testingDone === 'y') {
        const integrationWorking = await askQuestion(
          '8. Is the Linear MCP integration working? (y/n): '
        );
