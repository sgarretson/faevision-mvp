  ...
  try {
    ...
    for (const executiveData of PRODUCTION_EXECUTIVES) {
      try {
        ...
        if (existingUser) {
          console.log(`👤 User ${executiveData.email} already exists, skipping...`)
          continue
        }
        ...
        console.error(`❌ Failed to create user ${executiveData.email}:`, error)
      }
    }
      ...
      if (!existingInput) {
        await prisma.input.create({
          data: {
            ...
            description: 'This is a sample strategic input to demonstrate the F1-F6 executive workflow. FAEVision helps executives capture, collaborate on, organize, and solve strategic challenges efficiently.',
            department: 'STRATEGY',
            issueType: 'PROCESS',
            rootCause: 'Need for strategic management platform',
            priority: 'MEDIUM',
            businessImpact: 'Improved executive decision-making and strategic execution',
            status: 'ACTIVE',
    ...
    const executiveCount = await prisma.user.count({ where: { role: 'EXECUTIVE' } })
    ...
    console.log(`✅ Dashboard query executed successfully (${dashboardQuery.length} results)`)
    ...
    console.log('⚠️  IMPORTANT: All users should change their passwords on first login!')
