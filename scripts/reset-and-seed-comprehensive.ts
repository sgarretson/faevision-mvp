 ...
 *
 * This script implements our comprehensive seeding strategy:
 * 1. Reset preview database to clean state
 * 2. Create organizational foundation (departments, teams, users)
 * 3. Generate 50 properly tagged strategic inputs
 * 4. Process through enhanced AI pipeline
 * 5. Create complete workflow validation data
 *
...
async function main() {
  ...
  try {
    ...
    console.log('📊 Step 3: Creating 50 strategic inputs...');
    const signals = await createStrategicInputs(orgData);
    console.log(`✅ Created ${signals.length} strategic inputs\n`);

    // Step 4: Enhanced AI processing (we'll trigger this via API)
    console.log(
      '🤖 Step 4: Enhanced AI processing will be triggered via API...'
    );
    console.log('✅ Seeding foundation ready for AI processing\n');

    console.log('🎉 Comprehensive seeding completed successfully!');
    console.log('\n📋 Next Steps:');
    console.log('1. Run enhanced AI tagging via API endpoints');
    console.log('2. Execute clustering algorithm');
    console.log('3. Validate workflow: Signals → Hotspots → Ideas → Solutions');
  } catch (error) {
...
async function createStrategicInputs(orgData: any) {
  ...
  const communicationSignals = await Promise.all([
    ...
    // Add 10 more communication signals...
  ]);
