...
async function main() {
  console.log('🔍 COMPREHENSIVE API AUDIT - PRISMA TABLE NAME STANDARDIZATION\n');
  console.log('👥 Expert Team: Morgan Smith (Database) + Alex Thompson (Lead Dev)\n');
  ...
  console.log('🚀 All API routes standardized for Preview environment reliability');
}
    ...
    Object.entries(TABLE_NAME_PATTERNS).forEach(([wrong, correct]) => {
      ...
      patterns.forEach(pattern => {
        if (content.includes(pattern)) {
          result.issues.push(`Uses ${pattern} (should be ${pattern.replace(wrong, correct)})`);
          result.fixes.push(`Replace ${wrong} -> ${correct} in Prisma calls`);
          result.status = result.status === 'clean' ? 'needs_fix' : result.status;
        }
      });
    });
...
async function applyFixes(result: AuditResult): Promise<void> {
  ...
  try {
    ...
    Object.entries(TABLE_NAME_PATTERNS).forEach(([wrong, correct]) => {
      const operations = ['findMany', 'findFirst', 'findUnique', 'count', 'create', 'update', 'delete', 'upsert'];
      ...
      operations.forEach(op => {
        ...
        if (content.includes(wrongPattern)) {
          content = content.replace(new RegExp(wrongPattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), correctPattern);
          changesMade++;
        }
      });
    });
