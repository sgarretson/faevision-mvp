...
function validateDatabaseUrl(envName, databaseUrl, expectedHost) {
  ...
  if (databaseUrl.includes(expectedHost)) {
    ...
    console.log(`   ❌ Host verification: Expected ${expectedHost}, but not found`);
    return false;
  }
}
...
async function testDatabaseConnection(envName, envVars) {
  ...
  try {
    ...
    if (result.includes('Database schema is up to date')) {
      ...
      console.log(`   ⚠️  Database connection: Successful but needs migrations`);
...
async function testEnvironmentUrl(envName, url) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`   ⚠️  URL response: ${response.status} ${response.statusText}`);
...
async function generateTestReport(results) {
  ...
  Object.entries(results).forEach(([envKey, result]) => {
    ...
    console.log(`${envConfig.color} ${envConfig.name}: ${result.success ? '✅ PASS' : '❌ FAIL'}`);
  ...
  if (!overallPass) {
    ...
    console.log('1. Check Vercel environment variables are correctly configured');
    console.log('2. Verify Environment Scope is set properly for each variable');
...
async function main() {
  try {
    ...
    console.log('📊 Checking: Variables, Database Connections, Deployment URLs');
