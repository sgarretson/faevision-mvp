...
const openaiKey =
  productionEnv.match(/OPENAI_API_KEY="(.+)"/)?.[1] || 'MISSING';
const resendKey =
  productionEnv.match(/RESEND_API_KEY="(.+)"/)?.[1] || 'MISSING';

console.log('📊 Production API Keys Status:');
console.log(`   OPENAI_API_KEY: ${openaiKey.substring(0, 20)}...`);
console.log(
  `   RESEND_API_KEY: ${resendKey === 'MISSING' ? 'MISSING - Need to configure' : 'Found'}`
);
...
const previewEnvVars = {
  ...
  NEXT_PUBLIC_STACK_PUBLISHABLE_CLIENT_KEY:
    'pck_qh3h88mk87agj6mhsbez7hmjdg2a6bqjjpmf6e7t1hmrr',
  STACK_SECRET_SERVER_KEY: 'ssk_egev41977wwdrark62zba0xh6p3hd5cwat88s124xez0g',

  // API Keys (from production)
  OPENAI_API_KEY: openaiKey,
  RESEND_API_KEY:
    resendKey === 'MISSING' ? 'your-resend-api-key-here' : resendKey,
};
...
const developmentEnvVars = {
  // Database Configuration
  DATABASE_URL:
    'postgresql://neondb_owner:npg_6vcJmQuOek5X@ep-lingering-queen-ae13d5gh-pooler.c-2.us-east-2.aws.neon.tech/neondb?sslmode=require',
  DIRECT_URL:
    'postgresql://neondb_owner:npg_6vcJmQuOek5X@ep-lingering-queen-ae13d5gh.c-2.us-east-2.aws.neon.tech/neondb?sslmode=require',
  ...
  NEXT_PUBLIC_STACK_PUBLISHABLE_CLIENT_KEY:
    'pck_eg6nm35shw8z4psp0sz7xykam4n6smcgybxqke7b33dj0',
...
function displayEnvironmentConfiguration(envName, envVars, color) {
  ...
  console.log(
    'Copy these EXACTLY to Vercel Dashboard → Project Settings → Environment Variables:'
  );
...
function createVercelConfigurationGuide() {
  ...
  console.log(
    '1. Force redeploy Preview: git push origin preview --force-with-lease'
  );
  console.log(
    '2. Force redeploy Development: git push origin develop --force-with-lease'
  );
  console.log(
    '3. Test Preview URL: https://faevision-simplified-git-preview.vercel.app'
  );
  console.log(
    '4. Test Development URL: https://faevision-simplified-git-develop.vercel.app'
  );
...
function exportEnvironmentFiles() {
  ...
  fs.writeFileSync(
    '.env.preview.updated',
    `# FAEVision Preview Environment - Updated with Real API Keys\n# Generated: ${new Date().toISOString()}\n\n${previewContent}\n`
  );
  ...
  fs.writeFileSync(
    ...
    `# FAEVision Development Environment - Updated with Real API Keys\n# Generated: ${new Date().toISOString()}\n\n${developmentContent}\n`
  );
...
async function main() {
  try {
    ...
    console.log(
      '📊 Current Status: Preview & Development databases configured'
    );
    console.log(
      '🔧 Task: Set up Vercel environment variables for proper isolation'
    );
    ...
    console.log('\n📋 NEXT STEPS:');
    console.log(
      '1. 👤 USER: Configure variables in Vercel Dashboard (manual step)'
    );
    console.log('2. 🚀 TEAM: Force redeploy both environments to test');
    console.log(
      '3. ✅ VERIFY: Database connections working in both environments'
    );
