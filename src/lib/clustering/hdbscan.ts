...
export async function clusterSignalsHDBSCAN(
  ...
  try {
    ...
    const clustersWithStrengths = calculateMembershipStrengths(
      clusters,
      distanceMatrix
    );
    ...
    console.log(
      `    ✅ Clustering complete: ${clustersWithStrengths.length} clusters in ${duration}ms`
    );
...
function findCorePoints(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const kthDistance =
      distances[Math.min(options.minSamples - 1, distances.length - 1)]?.dist ||
      Infinity;
...
function buildClusters(
  ...
  for (const corePoint of Array.from(corePoints)) {
    ...
    while (queue.length > 0) {
      ...
      const reachablePoints = findReachablePoints(
        point,
        distanceMatrix,
        corePoints,
        options
...
function findReachablePoints(
  ...
  const epsilon =
    options.epsilon ||
    calculateDynamicEpsilon(coreDistances, options.minSamples);
 ...
 */
function calculateMembershipStrengths(
  clusters: Cluster[],
  distanceMatrix: number[][]
): Cluster[] {
  return clusters.map(cluster => {
    const membershipStrengths = cluster.points.map(pointIndex => {
      ...
      const avgDistance =
        otherPoints.reduce(
          (sum, otherPoint) => sum + distanceMatrix[pointIndex][otherPoint],
          0
        ) / otherPoints.length;

      // Convert distance to membership strength (0-1, where 1 is strongest)
      const maxDistance = Math.max(
        ...otherPoints.map(p => distanceMatrix[pointIndex][p])
      );
 ...
 */
function calculateClusterThreshold(
  pointIndices: number[],
  distanceMatrix: number[][]
): number {
