...
import { generateSolutionSuggestions, analyzeHotspotPotential } from '@/lib/ai/signal-processing';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (let i = 0; i < clusters.length; i++) {
      ...
      const clusterSignals = cluster.points.map(pointIndex => signals[pointIndex]);
      
      console.log(`    🔍 Analyzing cluster ${i} with ${clusterSignals.length} signals...`);
      ...
      try {
        const hotspot = await createHotspotFromCluster(clusterSignals, cluster, hotspotAnalysis);
        ...
        console.error(`      ❌ Failed to create hotspot from cluster ${i}:`, error);
      }
    }
    ...
    console.log(`  🎉 Hotspot generation complete: ${hotspotsCreated.length} created in ${duration}ms`);
    ...
    return NextResponse.json({
      ...
      clustersRejected: hotspotsAnalyzed.filter(h => !h.analysis.should_group).length,
      results: {
 ...
 */
async function createHotspotFromCluster(signals: any[], cluster: any, analysis: any) {
 ...
 */
function calculateAdvancedHotspotRank(signals: any[], analysis: any, cluster: any): number {
  ...
  const avgAge = signals.reduce((sum, signal) => {
    const age = Date.now() - new Date(signal.receivedAt || signal.createdAt).getTime();
    return sum + age;
  }, 0) / signals.length;
  ...
  const finalScore = (
    ...
    0.20 * volumeScore +     // Volume indicates systemic issues
    0.20 * aiConfidence +    // AI analysis quality
    0.15 * cohesionScore +   // Cluster quality
    0.10 * impactScore +     // Business impact
    0.10 * recencyScore      // Recency bonus
  );
...
export async function GET() {
  return POST(new NextRequest('http://localhost/api/cluster/generate-hotspots', { 
    method: 'POST',
