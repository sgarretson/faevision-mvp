...
export async function GET(request: NextRequest) {
  try {
    ...
    const [clusteringMetrics, aiMetrics, systemMetrics, businessMetrics] =
      await Promise.all([
...
async function getClusteringPerformance() {
  try {
    ...
    const avgSignalsPerHotspot =
      recentHotspots.length > 0
        ? (recentHotspots || []).reduce(
            (sum: number, h: any) => sum + h.signals.length,
            0
    ...
    const avgProcessingTime =
      Math.round((avgSignalsPerHotspot * 0.1 + 2) * 100) / 100;

    // Calculate success rate (confidence > 0.6)
    const successRate =
      recentHotspots.length > 0
        ? ((recentHotspots || []).filter((h: any) => h.confidence > 0.6)
            .length /
            (recentHotspots || []).length) *
          100
        : 100;
    ...
    return {
      ...
      avgConfidence:
        recentHotspots.length > 0
          ? (recentHotspots || []).reduce(
              (sum: number, h: any) => sum + h.confidence,
              0
            ) / (recentHotspots || []).length
          : 0,
    };
...
async function getAIPerformance() {
  try {
    ...
    const recentSignals = await (prisma as any).signal
      ?.findMany({
        where: {
          receivedAt: {
...
async function getBusinessMetrics() {
  try {
    ...
    // Solutions created
    const solutionsCreated = await (prisma as any).solutions.count({
      where: {
    ...
    // Hotspots resolved
    const hotspotsResolved = await (prisma as any).hotspot
      ?.count({
        where: {
          status: 'RESOLVED',
    ...
    solutions.forEach((solution: any) => {
      ...
      if (expected?.businessValue) {
        const value = parseFloat(
          expected.businessValue.replace(/[^0-9.-]+/g, '')
        );
        if (!isNaN(value)) {
          totalROI += value;
        }
