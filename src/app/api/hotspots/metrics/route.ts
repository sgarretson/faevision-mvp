...
async function getSignalMetrics() {
  try {
    ...
    const processingRate = totalSignals > 0 ? (processedSignals / totalSignals) : 0;
...
async function getClusteringMetrics() {
  try {
    ...
    const lastClusteringRun = recentHotspots.length > 0 
    ...
    const clusteringEfficiency = totalSignals > 0 ? (totalSignalsInHotspots / totalSignals) : 0;
