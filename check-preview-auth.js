...
async function checkPreviewAuth() {
  ...
  try {
    ...
    const testUsers = await prisma.user.findMany({
      where: {
        email: {
          in: ['admin@faevision.com', 'sarah.executive@faevision.com', 'alex.contributor@faevision.com']
        }
      },
      select: {
        id: true,
    ...
    if (usersWithoutPasswords > 0) {
      console.log(`⚠️  ${usersWithoutPasswords} users found without password hashes`);
    } else {
